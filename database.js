...
async function initDB() {
  ...
  // إنشاء/تحديث admin
  const adminRes = await db.execute("SELECT id,email FROM users WHERE role='admin' LIMIT 1");
  ...
  if (adminRes.rows.length === 0) {
    await db.execute({
      ...
      args: ['misha', ADMIN_EMAIL, hash, 'admin', 'Misha']
    });
    console.log('✅ Admin created: ' + ADMIN_EMAIL);
  } else {
    // تحديث كلمة السر والإيميل دائماً لضمان التزامن
    await db.execute({
      sql: "UPDATE users SET email=?, password=?, username='misha', display_name='Misha' WHERE role='admin'",
...
const q = {
  // ── Sessions (مخزنة في Turso لتعمل مع Vercel serverless) ──
  ...
  searchUsers:      (q) => db.execute({ sql:"SELECT id,username,display_name,avatar,role FROM users WHERE username LIKE ? OR display_name LIKE ? LIMIT 10", args:['%'+q+'%','%'+q+'%'] }).then(rows),
  ...
  // ── Reactions ──
  getReactions:    (record_id) => db.execute({ sql:'SELECT emoji, COUNT(*) as count FROM record_reactions WHERE record_id=? GROUP BY emoji', args:[record_id] }).then(rows),
  getUserReaction: (record_id,user_id) => db.execute({ sql:'SELECT emoji FROM record_reactions WHERE record_id=? AND user_id=?', args:[record_id,user_id] }).then(first),
  addReaction:     (record_id,user_id,emoji) => db.execute({ sql:'INSERT OR REPLACE INTO record_reactions (record_id,user_id,emoji) VALUES (?,?,?)', args:[record_id,user_id,emoji] }),
  removeReaction:  (record_id,user_id) => db.execute({ sql:'DELETE FROM record_reactions WHERE record_id=? AND user_id=?', args:[record_id,user_id] }),

  // ── Comments ──
  getComments:   (record_id) => db.execute({ sql:'SELECT * FROM record_comments WHERE record_id=? ORDER BY created_at ASC', args:[record_id] }).then(rows),
  addComment:    (record_id,user_id,username,display_name,avatar,user_role,content) => db.execute({ sql:'INSERT INTO record_comments (record_id,user_id,username,display_name,avatar,user_role,content) VALUES (?,?,?,?,?,?,?)', args:[record_id,user_id,username,display_name,avatar,user_role,content] }),
  deleteComment: (id,user_id,role) => role==='admin'
    ? db.execute({ sql:'DELETE FROM record_comments WHERE id=?', args:[id] })
    : db.execute({ sql:'DELETE FROM record_comments WHERE id=? AND user_id=?', args:[id,user_id] }),

  // ── Messages ──
  getConversations: (user_id) => db.execute({ sql:`
    ...
    ORDER BY m.created_at DESC`, args:[user_id,user_id,user_id] }).then(rows),
  getMessages:    (user_id,other_id) => db.execute({ sql:'SELECT m.*,u.avatar as from_avatar FROM messages m JOIN users u ON u.id=m.from_id WHERE (from_id=? AND to_id=?) OR (from_id=? AND to_id=?) ORDER BY created_at ASC', args:[user_id,other_id,other_id,user_id] }).then(rows),
  sendMessage:    (from_id,to_id,from_name,to_name,content) => db.execute({ sql:'INSERT INTO messages (from_id,to_id,from_name,to_name,content) VALUES (?,?,?,?,?)', args:[from_id,to_id,from_name,to_name,content] }),
  markRead:       (from_id,to_id) => db.execute({ sql:'UPDATE messages SET read=1 WHERE from_id=? AND to_id=?', args:[from_id,to_id] }),
  unreadCount:    (user_id) => db.execute({ sql:'SELECT COUNT(*) as count FROM messages WHERE to_id=? AND read=0', args:[user_id] }).then(first),
};
